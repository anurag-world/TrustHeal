...
export default function PatientCancelAppointments() {
  ...
  useEffect(() => {
    const LoadData = async () => {
      ...
      setisLoading(true);
      await axios
        .get(`${apiConfig.baseUrl}/patient/upcoming/consultations?patientId=${x.patientId}`)
        .then((response) => {
          console.log(
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            if (shouldShowCancel(item)) {
              Alert.alert(
                'Cancel Booking',
                `Dear ${patientDet.patientName},

We noticed you want to cancel your upcoming appointment. Please consider the appointment's importance before making changes, as your health is our top priority. 

If you wish to reschedule, please contact our customer care team.

If you still want to cancel, please note our policy requires 24 hours' notice for any changes. Check our cancellation policy.

Thank you for choosing TrustHeal to manage your appointments.
...
`,
              ...
              Alert.alert('Sorry!', 'You can not cancel the booking now.', [
                {
                  text: 'ok',
                  onPress: async () => {
                    // await getUpcoming();
        ...
        >
          ...
          />
          <Text style={{ fontSize: 12, color: 'white' }}>Cancel Booking</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <HeaderPatient showMenu={false} title="Cancel Consultations" />
          ...
          {!isEmpty(CompletedData) ? (
            ...
            <Text style={{ alignSelf: 'center', color: 'gray', marginVertical: 30 }}>
              No Data Found for Upcoming Consulations
            </Text>
          )}
        </ScrollView>
